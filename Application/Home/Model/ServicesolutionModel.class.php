...
class ServicesolutionModel extends Model {
	// 自动验证设置
	protected $_validate	 =	 array(	
		array('name','checkStringLength_name','故障修复描述不能为空，并且长度必须在 1-100 之间',1,'callback'), 
		array('name','checkUnique_name','故障修复描述重复',1,'callback'),  		
	);
	// 自动完成
   ...
   protected function checkUnique_name(){   //判断唯一性	
       return checkVarUnique('servicesolution','name')  ;    
   } 
     
   protected function checkStringLength_name(){   //判断字符串的长度是否在一个范围内  
	   return checkStringLength('name','1','255');    
   }  
