...
//                                    preg_match_all("/\bm.*\.(jpg|tif)/", $str, $matches);
    ...
    {
        $cate = M('goodscate');
        $map['id'] = I('post.cate_id');
        $map['company_id'] = session('company_id');
        $map['branch_id'] = session('branch_id');
        $map1['cate_id'] = I('post.cate_id');
        $map1['company_id'] = session('company_id');
        $map1['branch_id'] = session('branch_id');
        $model = $cate->field('model_mtl,model_obj')->where($map)->find();
        if (!empty($model)) {
            // 删除模型文件和本地文件
            if (unlink($model['model_mtl']) && unlink($model['model_obj'])) {
                $data['model_mtl'] = '';
                $data['model_obj'] = "";
                $cate->where($map)->save($data);

                $mtl = M('mtl_title');
                //删除mtl相关的数据
                if (!$mtl->where($map1)->delete()) {
                    $this->ajaxReturn(1, 'mtl文件数据删除失败', 1);
                    exit;
                }
            } else {
                $this->ajaxReturn(1, '模型文件删除失败', 1);
                exit;
            }
            // 删除map表中的贴图数据和本地文件
            $maptable = M('map');
            $map_path = $maptable->field('map_path')->where($map1)->select();
            foreach ($map_path as $k => $v) {
                if (!empty($map_path[$k]['map_path'])) {
                    unlink($map_path[$k]['map_path']);
                }
            }
            $maptable->where($map1)->delete();

            $this->ajaxReturn(1, '删除成功', 1);
        } else {
            $this->ajaxReturn(1, '该商品下还没有上传模型文件', 1);
        }
    }
