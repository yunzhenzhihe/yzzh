...
namespace Think;
/**
* 页面压缩,源码来源于CI  2016-4-19  郑杰修改了js的bug  114行
*/
class Minify {
...
static function compress($output, $type = 'text/html')
  {
    switch ($type)
    {
      case 'text/html':
        ...
        preg_match_all('{<script.+</script>}msU', $output, $javascript_clean);   //2016-04-18 郑杰注释掉的 会出现页面的js段落被删除的情况
 ...
 //   $output = preg_replace('!/\*([^/][^*]*\*)*/(?!.+?["\'])!i', '', $output);
    //2016-04-19 郑杰 修改 新加了下面的两行 如果script  运行下面的这行 就会输出空
	if ( strpos(strtolower($open_tag), 'script') == FALSE) {
		$output = preg_replace('!/\*([^/][^*]*\*)*/(?!.+?["\'])!i', '', $output);
	}
