		...
		$("#yt_table tr").each(function(trindex,tritem){  //遍历表格行  onclick="JavaScript:save_record() "  >
            if  ( trindex!=0 ) {
			 ...
			 if  (str.length==1) { str=str+"{"; }  else { str=str+",{"; }
			     ...
			     str=str+"'goods_memo':'"+ trim( $(tritem).find('td:eq(10)').html() )  +  "',";				 
				 str=str+"'regular_maintenance':'"+ trim(  $(tritem).find('td:eq(11)').html() )  + "',";
				 str=str+"'regular_visits':'"+ trim(  $(tritem).find('td:eq(12)').html() )  + "',";
				 str=str+"'warranty_period':'"+ trim(  $(tritem).find('td:eq(13)').html() )  + "',";	
				 str=str+"'warranty_period_txt':'"+ trim(  $(tritem).find('td:eq(14)').html() )  + "',";	
				 str=str+"'property':'"+ trim(  $(tritem).find('td:eq(15)').html() )  + "'";					 
			 str=str+"}";			 
			}  
	    });	 		
	   j=j.toFixed(2) ;		
