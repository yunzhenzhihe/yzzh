...
function change(v){
	var str="0";
	if (( v==2 )|| ( v==3 ) ) { var timename=setTimeout("YTmap_citytolnglat();",200); };  //定时 打开 倒计时器 
	for(i=0;i<v;i++){
	...
	with(ss){
		...
		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
			...
			//	if(v){ options[0].selected = true; }  // 原始的处理方式  用 select2me 以后 就不能用这条语句
				if(v){ 
				    $( ss ).select2("val",$(options[0]).text());
					$( '#s_county' ).select2("val",'市、县');
...
function _init_area(){  //初始化函数
	for(i=0;i<s.length;i++){  //原来是   for(i=0;i<s.length-1;i++){   最后一个县没有事件
