...
//----------------------------上传所有文件------------------------------------
        ...
        $abortBtn.click(function () { // 所有失败的请求
            thisS.abortAll();
        });
    ...
    Ssi_upload.prototype.toUploadFiles = function (files) {
        if (typeof this.options.maxNumberOfFiles === 'number') {
            if ((this.inProgress + this.pending) >= this.options.maxNumberOfFiles) {// 如果正在进行中，文件+挂起的文件超过我们定义的文件数，即文件的最大数量。
        ...
        for (var i = 0; i < filesLength; i++) {
            ...
            if ($.inArray(ext, this.options.allowed) === -1) { //如果请求的文件不允许
                ...
                if ($.inArray(ext, extErrors) === -1) {//如果我们第一次看到这个扩展
                    extErrors.push(ext); //把它推到exterrors变量
                }
                ...
                setupReader(file);
                this.pending++; // 我们更多的文件待上传
                this.currentListLength++;// 我们更多的文件在这个列表中
            } else {
                if (length > filesLength) {
                    filesLength++;
                } else {
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        if (this.pending > 0) {
            ...
            var val = $uploadBtn.parents('th').prev().prev().html();//获取字段值
            var cate_id = $uploadBtn.parents('th').prev().html();//获取cate_id
            var id = $uploadBtn.parents('th').prev().prev().html();
        ...
        function ajaxLoopRequest(formData, ii) {
                ...
                xhr: function () {
                    ...
                    xhr.upload.addEventListener('progress', function (e) {// add event listener to progress
                        if (e.lengthComputable) {
                            var percentComplete = (e.loaded / e.total) * 100;// 计算进度
                            if (uploadBar) {
                                uploadBar.css({
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                if (typeof thisS.options.onEachUpload === 'function') {//执行onEachUpload回调
                    try {
                        thisS.options.onEachUpload({//and return some info
    ...
    //functions
    String.prototype.replaceText = function () {//用变量替换$
    ...
    String.prototype.getExtension = function () {//返回路径或文件的扩展名。
    ...
    var arraySum = function (arr) {//返回数组的和
        var sum = 0;
    ...
    var locale = {
        ...
        cn: {
            ...
            clear: '清除',
            drag: '将文件拖拽到此区域',
            sizeError: '$1 超过 $2 的大小限制。',// $1=file name ,$2=max file size ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 文件格式不支持',// $1=file extension ie(exe files are not supported)
